---
title: Конфигурация
description: Полный справочник по tyndale.config.json.
---

## Файл конфигурации

Создайте `tyndale.config.json` в корне вашего проекта или выполните `npx tyndale init`, чтобы сгенерировать его.

```json
{
  "defaultLocale": "en",
  "locales": ["es", "fr", "ja"],
  "source": ["src", "app"],
  "extensions": [".ts", ".tsx", ".js", ".jsx"],
  "output": "public/_tyndale",
  "translate": {
    "tokenBudget": 50000,
    "concurrency": 8
  },
  "localeAliases": { "pt-BR": "pt" },
  "dictionaries": {
    "include": ["src/dictionaries/*.json"],
    "format": "key-value"
  },
  "pi": {
    "model": "claude-sonnet-4-20250514",
    "thinkingLevel": "low"
  },
  "docs": {
    "framework": "starlight",
    "contentDir": "src/content/docs"
  }
}
```

## Поля

| Поле | Тип | По умолчанию | Описание |
|-------|------|---------|-------------|
| `defaultLocale` | `string` | `"en"` | Код исходного языка |
| `locales` | `string[]` | `[]` | Коды целевых локалей |
| `source` | `string[]` | `["src"]` | Каталоги для сканирования на наличие переводимого контента |
| `extensions` | `string[]` | `[".ts", ".tsx", ".js", ".jsx"]` | Включаемые расширения файлов |
| `output` | `string` | `"public/_tyndale"` | Каталог вывода для сгенерированных файлов локалей |
| `translate.tokenBudget` | `number` | `50000` | Бюджет токенов на один пакет AI-перевода |
| `translate.concurrency` | `number` | auto | Максимальное число параллельных сессий перевода |
| `localeAliases` | `object` | `{}` | Сопоставление кодов вариантов локалей с каноническими |
| `dictionaries.include` | `string[]` | `[]` | Glob-шаблоны для файлов словарей |
| `dictionaries.format` | `string` | `"key-value"` | Формат файла словаря |
| `pi.model` | `string` | `"claude-sonnet-4-20250514"` | Используемая AI-модель |
| `pi.thinkingLevel` | `string` | `"low"` | Уровень рассуждений AI |
| `docs.framework` | `string` | — | Фреймворк документации: `starlight`, `docusaurus`, `vitepress`, `mkdocs`, or `nextra` |
| `docs.contentDir` | `string` | auto | Каталог контента (относительно корня проекта) |
| `docs.extensions` | `string[]` | auto | Переопределяет расширения файлов для перевода документации |